...
class CHMPipeline(BasePipeline):
    ...
    def train_old_old(self):
        ...
        # Build optimizer - what you use to optimize the batch gradient - the loss function uses the optimizer
    ...
    def train_old(self):
        ...
        if self.conf.model_name == 'dinov2_rs':
            ...
            # Load DinoV2 and create the pixel-wise regression model
            model_name = "facebook/dinov2-base"
            backbone_config = AutoConfig.from_pretrained(model_name)
            # Update the model configuration to expect 5 channels
            backbone_config.num_channels = len(self.conf.output_bands)
            print(backbone_config)

            backbone = AutoModel.from_pretrained("facebook/dinov2-base", config=backbone_config, ignore_mismatched_sizes=True)

            # Modify the patch embedding layer to accept 5 input channels
            # DinoV2 uses `num_channels` to represent the number of input channels
            original_patch_embeddings = backbone.embeddings.patch_embeddings

            # Create a new convolutional layer with 5 input channels
            new_conv = nn.Conv2d(
                in_channels=len(self.conf.output_bands),
                out_channels=original_patch_embeddings.projection.out_channels,
                kernel_size=original_patch_embeddings.projection.kernel_size,
                stride=original_patch_embeddings.projection.stride,
                padding=original_patch_embeddings.projection.padding,
                bias=original_patch_embeddings.projection.bias is not None
            )

            # Split the original weights and concatenate with new initialized weights
            with torch.no_grad():
                # Extract the first 3 channels from the original weights
                original_weights = original_patch_embeddings.projection.weight[:, :len(self.conf.output_bands), :, :]

                # # Initialize weights for the additional 2 channels
                # extra_weights = torch.randn_like(original_weights[:, :2, :, :]) * 0.02  # Random init with small std deviation # TODO: 2 channels? why?

                # # Concatenate the original weights with the extra weights along the channel dimension
                # combined_weights = torch.cat([original_weights, extra_weights], dim=1)
                combined_weights = original_weights

                # Assign the concatenated weights to the new convolution layer
                new_conv.weight = nn.Parameter(combined_weights)

            # Replace the original projection layer with the modified one
            backbone.embeddings.patch_embeddings.projection = new_conv

            # Update the model configuration to expect 5 channels
            backbone.config.num_channels = len(self.conf.output_bands)

            model = DinoV2PixelwiseRegressionModel(backbone)
            pixelwise_regression_model = PixelwiseRegressionModel(model)

            print("Expected channels:", backbone.config.num_channels)
            print("Patch embedding layer input channels:", backbone.embeddings.patch_embeddings.projection.in_channels)
            ...
            # Define loss and optimizer for pixel-wise regression
            ...
            # Define trainer with desired configuration
            num_epochs = self.conf.num_epochs # TODO: this should be exposed as a var in config
            # This is a warm-up - but its model dependent 
            trainer = pl.Trainer(max_epochs=self.conf.num_epochs_warmup, accelerator="gpu")  # Run for 1 epoch each loop iteration

            # Training loop across multiple rounds of training
            total_epochs = self.conf.num_epochs # TODO: this should be exposed as a var in config
            for epoch in range(0, total_epochs, num_epochs):
                print(f"Training epochs {epoch + 1} to {epoch + num_epochs}")
                trainer.fit(pixelwise_regression_model, datamodule=datamodule)
